...
from tts.tacotron2.attention import Attention
from tts.tacotron2.modules import XavierLinear
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.mel_out = XavierLinear(
            rnn_hidden_dim + embedding_dim, num_mels, nonlinearity="linear"
        )

        # Final layer producing gate output
        self.gate = XavierLinear(
            rnn_hidden_dim + embedding_dim, 1, nonlinearity="sigmoid"
        )
    ...
    def forward(
        ...
        Args:
            ...
            encoded -- raw output from the encoder
